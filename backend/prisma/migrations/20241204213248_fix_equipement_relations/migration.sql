...
-- CreateTable
CREATE TABLE `_AdminEquipement` (
    `A` INTEGER NOT NULL,
    `B` INTEGER NOT NULL,

    UNIQUE INDEX `_AdminEquipement_AB_unique`(`A`, `B`),
    INDEX `_AdminEquipement_B_index`(`B`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `_AdminEquipement` ADD CONSTRAINT `_AdminEquipement_A_fkey` FOREIGN KEY (`A`) REFERENCES `Admin`(`Id_Utilisateur`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `_AdminEquipement` ADD CONSTRAINT `_AdminEquipement_B_fkey` FOREIGN KEY (`B`) REFERENCES `Equipement`(`id_Equipement`) ON DELETE CASCADE ON UPDATE CASCADE;
